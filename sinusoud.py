...
class Triangle(Sinusoid):
  def __init__(self, frequency=440, phase=0, amplitude=1.0, width=0.5):
    self.width = width
  ...
  def sample(self, duration, sample_rate, start_time=0.0):
    ...
    cycles = self.frequency * t + self.phase / 2 * np.pi
    frac, _ = np.modf(cycles)
    samples = np.abs(frac - self.width)
    samples = np.interp(
      samples,
      (0, self.width if self.width > 0 else 1),
      (-1, 1)
    )
